...
n_random_riders = param_form.slider("Number of Park Guests (Random Strategy)", min_value=1, max_value=1000)
n_queue_length_riders = param_form.slider("Number of Park Guests (Queue length Strategy)", min_value=0, max_value=1000)
n_itinerary_riders = param_form.slider("Number of Park Guests (Itinerary Strategy", min_value=0, max_value=1000)
...
time_between_ticks = param_form.slider("Time in between ticks (ms)", min_value=50, max_value=1000, step=50)
submitted = param_form.form_submit_button("Submit")

attractions = get_attractions_1()
park = Park(attractions)

guests = (
    [Guest(get_person_name()) for i in range(n_random_riders)]
    + [QueueLengthGuest(get_person_name()) for i in range(n_queue_length_riders)]
    + get_itinerary_guests(n_itinerary_riders, attractions)
)
...
with st.expander("Park Setup", expanded=True):
    ...
    for a in attractions:
        st.markdown(f"**{a.name}**: this ride seats {a.capacity}, and takes {a.duration_ticks} ticks to ride.")
